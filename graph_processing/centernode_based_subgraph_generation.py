...
# params:
config_file_path = "data/subgraphs/patho-report_subgraphs/patho-report-subgraph-config-1.0.yaml"
...
if __name__ == '__main__':
    ...
    if subgraph_generation_config["test_run"]:
        subgraph_name = (subgraph_generation_config["subgraph_name"] +
                         f"_p{subgraph_generation_config['max_path_length']}_r{subgraph_generation_config['neighborhood_extraction_size']}" + "_TEST")
    else:
        subgraph_name = (subgraph_generation_config["subgraph_name"] +
                         f"_p{subgraph_generation_config['max_path_length']}_r{subgraph_generation_config['neighborhood_extraction_size']}")
    ...
    center_node_id_prop = subgraph_generation_config["center_node_identification_property"]
    base_label = ":" + subgraph_generation_config["base_label"] if subgraph_generation_config["base_label"] else ""
    center_label = ":" + subgraph_generation_config['center_node_label'] if subgraph_generation_config['center_node_label'] else ""
    ...
    for center_node_id, target_node_id in tqdm(start_target_nodes):
        ...
        # find shortest possible path between center_node and target_node (is fast):
        query = f"MATCH p = shortestPath((a{center_label})-[rels*..{subgraph_generation_config['max_path_length']}]-(b{center_label})) " \
                f"WHERE a.{center_node_id_prop} = {center_node_id} AND b.{center_node_id_prop} = {target_node_id} " \
        ...
        if not paths:
            ...
            largest_path_length = max(largest_path_length, smallest_path_length)

            if not subgraph_generation_config["test_run"]:
                if subgraph_generation_config["collect_all_paths"]:
                    query = f"MATCH p = (a{center_label})-[rels*{smallest_path_length}..{subgraph_generation_config['max_path_length']}]-(b{center_label}) " \
                            f"WHERE a.{center_node_id_prop} = {center_node_id} AND b.{center_node_id_prop} = {target_node_id} " \
                            f"AND all(r IN rels WHERE {subgraph_generation_config['path_search_relation_condition']}) " \
                            f"RETURN p"
                    paths = [r["p"] for r in session.run(query)]
                else: # get all shortest paths (there might be multiple shortest paths):
                    query = f"MATCH p = (a{center_label})-[rels*{smallest_path_length}]-(b{center_label}) " \
                            f"WHERE a.{center_node_id_prop} = {center_node_id} AND b.{center_node_id_prop} = {target_node_id} " \
                            f"AND all(r IN rels WHERE {subgraph_generation_config['path_search_relation_condition']}) " \
                            f"RETURN p"
                    paths = [r["p"] for r in session.run(query)]
                #print(f"found {len(paths)} shortest paths between {center_node_id} and {target_node_id} with fixed length {smallest_path_length}")
    ...
    if subgraph_generation_config["label_center_nodes_with"]:
        ...
        for center_node_id in center_node_ids:
            session.run(f"match (a{center_label}) where a.{center_node_id_prop} = {center_node_id} set a:{subgraph_generation_config['label_center_nodes_with']}")
    ...
    if neighborhood_extraction_size > 0:
        ...
        print(f"Adding neighborhood r={neighborhood_extraction_size} to the subgraph...")
        for center_node_id in tqdm(center_node_ids):
            query = f"MATCH (a{center_label})-[r*1..{neighborhood_extraction_size}]-(b) " \
                    f"WHERE a.{center_node_id_prop} = {center_node_id} " \
                    f"RETURN r"
    ...
    stats = {
        ...
        "length_of_largest_found_path": largest_path_length
    }
    print("Done.\nStats:")
    for k in stats:
        print(f" -{k}:\t{stats[k]}")
    ...
    for node_id in tqdm(selected_nodes):
        session.run(f"match (n) where (n{center_label} or n{base_label}) and ID(n) = {node_id} set n.{subgraph_name} = True")
    ...
    input_msg = input(f"Done. If you want to add additional nodes and edges with .{subgraph_name}=True to the subgraph, "
                      f"you can do this now. If you are done, press enter to continue, or enter 'exit' to abort.")
    ...
    for i_try in range(3): # this can fail sometimes (cased by wrong settings in neo4j db), so lets try it 3 times
        try:
            print("next, lets convert the rdf file to a graph object and load it into an empty neo4j db.\n"
                  ...
                  f" 5.1) open the neo4j-db terminal, 'cd conf', 'touch apoc.conf', 'nano apoc.conf'.\n"
                  f" 5.2) Set apoc.import.file.enabled=true in the apoc.conf file.\n"
            ...
            unique_prop = subgraph_generation_config["unique_identifier_of_base_nodes"]
            ids = [r["id"] for r in session.run(
                f"match (a{base_label}) where a.{unique_prop} IS NOT NULL return distinct a.{unique_prop} as id")]
            unique_id_count = len(ids)
            all_id_count = len([r for r in session.run(
                f"match (a{base_label}) where a.{unique_prop} IS NOT NULL return a.{unique_prop}")])

            print("done. Now waiting 10 seconds to let the db settle...")
            time.sleep(10)
            ...
            if all_id_count > unique_id_count:
                ...
                for id in tqdm(ids):
                    query = f"MATCH (a{base_label}) where a.{unique_prop} = '{id}' RETURN ID(a) as id"
                    dublicated_ids = [r["id"] for r in session.run(query)]
                    if len(dublicated_ids) > 1:
                        for i_d in range(1, len(dublicated_ids)):
                            q = f"MATCH (a{base_label}) where ID(a) = {dublicated_ids[i_d]} DETACH DELETE a"
                            session.run(q)
                            print(q)
